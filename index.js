...
bot.on('messageCreate', (msg) => {
      ...
      case "!linux":
        ...
        if(index === -1){
          ...
          bot.editGuildMember(msg.channel.guild.id, userID, {
            ...
            bot.createMessage(config.modLogChannel, "Gave `Linux Testers` to **" + userTag + "**");
          }).catch(console.error);
        }else{
          roles.splice(index, 1);
          bot.editGuildMember(msg.channel.guild.id, userID, {
            ...
            bot.createMessage(config.modLogChannel, "Removed `Linux Testers` from **" + userTag + "**");
          }).catch(console.error);
        }
      break;
    }
      ...
      t.get("/1/cards/" + trelloURL, { }, function(errorURL, urlData) {
        if(!!urlData && !!urlData.id && urlData.closed === false){
          if(note === trelloURL){
            ...
            repro(note, undefined, channelID, trelloURL, userID, userTag, undefined, undefined, msgID);
          }
        }else{
    ...
    if(command.toLowerCase() === "!attach"){
      ...
      if(dev > -1 || hunter > -1 || admin > -1){
        ...
        bot.createMessage(channelID, "<@" + userID + ">, only people with the role of Bug Hunter or higher can use that command, in order to prevent spam and abuse. Just ask any Bug Hunter™ and they'll be more than happy to help you out.").then(delay(config.delayInMS)).then((msg_id) => {
          bot.deleteMessage(msg_id.channel.id, msg_id.id).catch(() => {});
          bot.deleteMessage(channelID, msgID).catch(() => {});
        }).catch((err) => {
          console.log("#18 " + err);
        });
                    ...
                    if(section2[0].indexOf(' - ') > -1){
                      ...
                      if(!!msg.attachments[0]){
                        bot.createMessage(channelID, "<@" + userID + "> please use the attach command to attach images instead of embeding images to the edit report").then(delay(config.delayInMS)).then((innerMsg) => {
                          bot.deleteMessage(channelID, innerMsg.id).catch(() => {}).catch(() => {});
                        }).catch(console.error);
                      }
                      bot.getMessages(channelID).then((data) => {
                        ...
                        editTrelloCard(trelloURL, channelID, reportString, '<@' + userID + '>', userTag, msgID, fixedEditMessage, dataFinder.id);
                      }).catch((err) => {
              ...
              if(!!trelloURL && (newTitle !== trelloURL)){
                bot.getMessages(channelID).then((data) => {
                  ...
                  if(!!dataFinder){
                    ...
                    updateTrelloCardTitle(trelloURL, channelID, newTitle, '<@' + userID + '>', userTag, msgID, editReportString, dataFinder.id);
                  }else{
                    updateTrelloCardTitle(trelloURL, channelID, newTitle, '<@' + userID + '>', userTag, msgID, null, null);
                  }
                }).catch((err) => {
            ...
            if(!!matchFormat && matchFormat.indexOf('actual result') > -1){
              ...
              if(section2[0].indexOf(' - ') > -1){
                ...
                if(channelID === config.iosChannel){
                  ...
                  queueReport(reportStringSubmit, header, cleanRepostReport, channelID, userTag, msgID, config.iosChannel, listID, userID, msg.content);
                }else if(channelID === config.androidChannel){
                  var listID = config.androidCard;
                  queueReport(reportStringSubmit, header, cleanRepostReport, channelID, userTag, msgID, config.androidChannel, listID, userID, msg.content);
                }else if(channelID === config.canaryChannel){
                  var listID = config.canaryCard;
                  queueReport(reportStringSubmit, header, cleanRepostReport, channelID, userTag, msgID, config.canaryChannel, listID, userID, msg.content);
                }else if (channelID === config.linuxChannel) {
                  var listID = config.linuxCard;
                  queueReport(reportStringSubmit, header, cleanRepostReport, channelID, userTag, msgID, config.linuxChannel, listID, userID, msg.content);
                }
      ...
      if(!!contentMessage) {
        ...
        fs.writeFile('./dataFile.json', newDNUserCount, function(err){
          ...
          fs.writeFile("./dataFile.json", newStatus, function(err){
            bot.createMessage(channelID, "<@" + userID + ">, report has been denied").then(delay(config.delayInMS)).then(innerMsg => {
              bot.deleteMessage(channelID, innerMsg.id).catch(() => {});
              bot.deleteMessage(channelID, msgID).catch(() => {});
              bot.deleteMessage(config.bugApprovalChannel, info.msgID).then(() => {
                bot.createMessage(config.bugApprovalChannel, "`" + info.header + "` was denied for:\n- `" + info.denialReason.join('`\n- `') + "`").then(delay(config.minuteDelay)).then((msgInfo) => {
                  bot.deleteMessage(config.bugApprovalChannel, msgInfo.id).catch(() => {});
                }).catch(console.error);
              }).catch((error) => {
                console.log("ddeny | denial reasons\n" + error);
              });
            }).catch(console.error);
          });
        ...
        bot.createMessage(channelID, "<@" + userID + "> psst, you forgot the denial reason").then(delay(config.delayInMS)).then(innerMsg => {
          bot.deleteMessage(channelID, innerMsg.id).catch(() => {});
          bot.deleteMessage(channelID, msgID).catch(() => {});
        }).catch((err) => {
          console.log("ddeny | forgot reason\n" + err);
        });
      }
    }
  }
  ...
  if(command.toLowerCase() === "!dapprove" ){
    ...
    if(dev > -1 || admin > -1){
      ...
      if(!!contentMessage) {
        info.approvedBy++;
        info.hunterTag = [userTag];
        info.hunterReproSystem.push(contentMessage[2]);
        var forcedApproval = JSON.stringify(dataFile, null, 2);
        fs.writeFile('./dataFile.json', forcedApproval, function(err){
          let listID = info.listID,
              header = info.header,
          ...
          sendToTrello(listID, header, reportStringSubmit, loggedChannelID, os, loggedUserTag, cleanRepostReport, loggedUserID, uniqueID);
          ...
          bot.createMessage(channelID, "<@" + userID + "> you've successfully added your approval for this report. 3/3").then(delay(config.delayInMS)).then(innerMsg => {
            ...
            console.log("dapprove | approvedReport\n" + err);
          });
          ...
          fs.writeFile("./dataFile.json", newStatus, function(err){
            if(!!err){
              console.log("dapprove | fs WriteFile\n" + err);
            }
          });
        });
      }else {
        bot.createMessage(channelID, "<@" + userID + "> psst, you forgot your system specs").then(delay(config.delayInMS)).then(innerMsg => {
          bot.deleteMessage(channelID, innerMsg.id).catch(() => {});
          bot.deleteMessage(channelID, msgID).catch(() => {});
        }).catch((err) => {
          console.log("dapprove | forgot specs\n" + err);
        });
      }
    }
  }
  ...
  if(command.toLowerCase() === "!bug") {
    ...
    if(dev > -1 || admin > -1 || mod > -1) {
      ...
      if(!!uniqueID) {
        ...
        bot.createMessage(channelID, "<@" + userID + "> psssst, you're missing the report key!").then(delay(config.delayInMS)).then((msgInfo) => {
          bot.deleteMessage(channelID, msgID).catch(() => {});
...
function queueRepro(uniqueID, channelID, trelloURL){
  ...
  if(queueReproQueue > lengthOfRepro){
    queueReproQueue = 1;
  }else if(queueReproQueue <= lengthOfRepro && lengthOfRepro >= 1){
    ...
    if(!!urlData){
      ...
      if(preReproTries >= 5){
        ...
        bot.createMessage(channelID, "Something went wrong, please try again later.").then(delay(config.delayInMS)).then(innerMsg => {
          bot.deleteMessages(channelID, innerMsg.id).catch(() => {});
        }).catch((err) => {
          console.log("#45 " + err);
        });
        preReproTries = 0;
...
function sendToTrello(listID, header, report, channelID, whereFrom, userTag, repostReportString, loggedUserID, uniqueID){
...
function checkUserInfo(loggedUserID, userTag, whereFrom, shortUrl, uniqueID) {
  ...
  } else if(reTestUserInfo >= 5) {
    setTimeout(function() {
      reTestUserInfo++;
      console.log(guild);
      checkUserInfo(loggedUserID, userTag, whereFrom, shortUrl, uniqueID);
    }, 2000);
  } else {
